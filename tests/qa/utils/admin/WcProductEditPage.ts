...
export class WcProductEditPage extends WpPage {
	...
	// General tab
	generalTabLink = () =>
		this.page.locator( 'a[href="#product_data-general"]' );
	regularPriceInput = () => this.page.locator( '#_regular_price' );

	// Inventory tab
	inventoryTabLink = () =>
		this.page.locator( 'a[href="#product_data-inventory"]' );
	...
	getProductIdFromUrl = (): number => {
		...
		if ( ! match ) {
			throw new Error(
				`Could not extract product ID from URL: ${ this.page.url() }`
			);
		}
	...
	excludeFromSyncCheckbox = () =>
		this.page.locator( '#_zettle_exclude_from_sync' );
