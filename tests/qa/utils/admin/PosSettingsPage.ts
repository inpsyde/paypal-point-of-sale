...
export class PosSettingsPage extends WpPage {
	...
	authoriseHeading = () =>
		this.page.getByRole( 'heading', { name: 'Authorise connection' } );
	...
	authFailedHeading = () =>
		this.page.getByRole( 'heading', { name: 'Authentication failed' } );
	startOverButton = () =>
		this.page.getByRole( 'button', { name: 'Start over' } );
	...
	startSyncButton = () =>
		this.page.getByRole( 'button', { name: 'Start sync' } );
	...
	disconnectConfirm = () =>
		this.page.locator( '#zettleDisconnectModal button[name="delete"]' );
	// "Cancel" button inside the modal
	disconnectCancel = () =>
		this.page.locator( '#zettleDisconnectModal button[name="back"]' );
	disconnectModalHeading = () =>
		this.page.getByRole( 'heading', {
	...
	connect = async ( apiKey: string, cli: AnyCli ) => {
		...
		try {
			await this.page.waitForSelector( 'button[name="cancel"]', {
				state: 'detached',
				timeout: 180_000,
			} );
		} finally {
