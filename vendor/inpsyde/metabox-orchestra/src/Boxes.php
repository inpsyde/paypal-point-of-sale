...
use Syde\Vendor\Zettle\Brain\Nonces\WpNonce;
/**
...
class Boxes
{
    ...
    public function add_box(Metabox ...$boxes)
    {
        ...
        if (!$this->target->valid() || !in_array($this->registering_for, [Metabox::SAVE, Metabox::SHOW], \true)) {
            return $this;
        }
    ...
    public function print_term_boxes(\WP_Term $term)
    {
        ...
        $script = '!function(J,D){J(function(){J(".meta-box-sortables").sortable();' . 'J(D).on("click",".termbox-container button.handlediv",function(){' . 'var D=J(this),t=D.siblings(".inside");t.toggle();var e=t.is(":visible")?"true":"false";' . 'D.attr("aria-expanded",e)})})}(jQuery,document);';
    ...
    private function init_for_post(): bool
    {
        ...
        add_filter('wp_insert_post_empty_content', function ($empty, array $post_array) {
            global $post;
    ...
    private function init_for_term(string $taxonomy): bool
    {
        ...
        add_action('edit_term', function ($term_id, $term_taxonomy_id, $term_taxonomy) use ($taxonomy) {
            ...
            if (!$term instanceof \WP_Term || (int) $term->term_id !== (int) $term_id || $term->taxonomy !== $term_taxonomy || $term->taxonomy !== $taxonomy) {
                return;
            }
            $this->saving = 'term';
            $entity = new Entity($term);
    ...
    private function add_meta_box(Metabox $box, string $box_id)
    {
        ...
        add_meta_box($box_id . $box_suffix, $info->title(), static function ($object) use ($box_id, $view, $box, $info) {
            $object_id = $object instanceof \WP_Post ? $object->ID : $object->term_id;
            print \Syde\Vendor\Zettle\Brain\Nonces\formField(new WpNonce($box_id . "-{$object_id}"));
    ...
    {
        if (wp_is_post_autosave($post) || wp_is_post_revision($post) || is_multisite() && ms_is_switched()) {
            return;
        }
