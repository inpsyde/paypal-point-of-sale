...
class AdminNotices
{
     ...
     * @return AdminNotices
     */
    public function add(string $message, string $title = '', string $type = self::ERROR, string $target_screen = null): AdminNotices
    {
    ...
    public function do_notices(): bool
    {
        ...
        if (!empty($messages[$screen_id])) {
            ...
            return $messages ? (bool) update_user_option($user_id, self::OPTION_NAME, $messages) : (bool) delete_user_option($user_id, self::OPTION_NAME);
        }
    ...
    public function record(): bool
    {
        ...
        $user_id = get_current_user_id();
        if ($user_id) {
            return $this->messages ? (bool) update_user_option($user_id, self::OPTION_NAME, $this->messages) : (bool) delete_user_option($user_id, self::OPTION_NAME);
        }
        return \FALSE;
    }
    /**
