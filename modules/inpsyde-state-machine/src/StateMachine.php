...
class StateMachine implements StateMachineInterface
{
    ...
    public function apply($transition): StateMachineInterface
    {
        ...
        if (!$this->can($transition)) {
            throw new DenyTransitionException(sprintf("Current State %s can't make Transition %s", $this->currentState->name(), $transition instanceof TransitionInterface ? $transition->name() : $transition));
        }
        $oldState = $this->currentState->name();
        $newState = $this->getState($transition->toState());
        $this->eventDispatcher->dispatch(new GenericPreTransition($oldState, $transition));
        $this->setCurrentState($newState);
        $this->eventDispatcher->dispatch(new GenericPostTransition($oldState, $transition));
        return $this;
    }
    /**
     * @param string $transition
    ...
    public function handle($event)
    {
        ...
        $transition = array_filter($this->availableTransitions(), static function (TransitionInterface $transition) use ($targetState): bool {
