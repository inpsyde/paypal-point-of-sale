...
use Syde\Vendor\Zettle\Dhii\Data\Container\WritableContainerInterface;
...
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\Onboarding\OnboardingTransition as T;
...
return [
    ...
    'paypal-pos.onboarding.states' => static function (C $container): array {
        return [new State(S::WELCOME, State::TYPE_INITIAL), new State(S::API_CREDENTIALS), new State(S::INVALID_CREDENTIALS), new State(S::SYNC_PARAM_PRODUCTS), new State(S::SYNC_PARAM_VAT), new State(S::SYNC_PROGRESS), new State(S::SYNC_FINISHED), new State(S::ONBOARDING_COMPLETED), new State(S::UNHANDLED_ERROR)];
    },
    ...
    'paypal-pos.onboarding.transitions' => static function (C $container): array {
        return [new Transition(T::TO_API_CREDENTIALS, [S::WELCOME, S::SYNC_PARAM_PRODUCTS, S::INVALID_CREDENTIALS], S::API_CREDENTIALS), new Transition(T::TO_WELCOME, [S::API_CREDENTIALS, S::UNHANDLED_ERROR], S::WELCOME), new Transition(T::TO_INVALID_CREDENTIALS, [S::API_CREDENTIALS, S::SYNC_PARAM_PRODUCTS, S::SYNC_PARAM_VAT, S::SYNC_PROGRESS], S::INVALID_CREDENTIALS), new Transition(T::TO_SYNC_PARAM_PRODUCTS, [S::API_CREDENTIALS, S::SYNC_PARAM_VAT, S::SYNC_PROGRESS], S::SYNC_PARAM_PRODUCTS), new Transition(T::TO_SYNC_PARAM_VAT, [S::SYNC_PARAM_PRODUCTS, S::SYNC_PROGRESS], S::SYNC_PARAM_VAT), new Transition(T::TO_SYNC_PROGRESS, [S::SYNC_PARAM_VAT], S::SYNC_PROGRESS), new Transition(T::TO_SYNC_FINISHED, [S::SYNC_PROGRESS], S::SYNC_FINISHED), new Transition(T::TO_ONBOARDING_COMPLETED, [S::SYNC_FINISHED], S::ONBOARDING_COMPLETED), new Transition(T::TO_UNHANDLED_ERROR, array_diff($container->get('paypal-pos.onboarding.states.names'), [S::UNHANDLED_ERROR, S::ONBOARDING_COMPLETED]), S::UNHANDLED_ERROR)];
    },
    /**
        ...
        return [
            ...
            S::API_CREDENTIALS => [
                ...
                static function (ProceedButtonPressed $event) use ($container, $validateField) {
                    ...
                    if (!$validateField('woocommerce_zettle_api_key', $data)) {
                        $addErrorMessageCallback(esc_html__("Please enter a valid API key or create a new API Key, if you don't have one already.", 'paypal-point-of-sale'));
                    ...
                    if ($settings->has('api_key') && !empty($settings->get('api_key')) && !$container->get('paypal-pos.onboarding.settings.write-only-password-field-checker')($token)) {
                    ...
                    if (!($jwtValidator->validate($token) && $credentialValidator->validateApiToken($token))) {
                        $addErrorMessageCallback(esc_html__('The API key you entered is invalid. Try again or create a new API key if the problem still occurs.', 'paypal-point-of-sale'));
                        $event->transitionTo(S::INVALID_CREDENTIALS);
                        return;
                    }
                    $event->transitionTo(S::SYNC_PARAM_PRODUCTS);
            ...
            S::SYNC_PARAM_VAT => [
                ...
                static function (ProceedButtonPressed $event) use ($container) {
                    ...
                    if (!$syncTaxStrategy) {
                        $addErrorMessage(__('Missing price sync configuration!', 'paypal-point-of-sale'));
                        return;
                    }
                    ...
                    if ($syncTaxStrategy === PriceSyncMode::ENABLED && !$storeComparison->currency()) {
                        $addErrorMessage(wp_kses_post(__('Invalid price sync configuration!', 'paypal-point-of-sale')));
                        return;
                    }
        ...
        return [
            ...
            T::TO_ONBOARDING_COMPLETED => static function (PostTransition $event) use ($container) {
                ...
                try {
                    ...
                    $addErrorMessageCallback = $container->get('paypal-pos.onboarding.message.add.error');
                    $addErrorMessageCallback(wp_kses_post($container->get('paypal-pos.onboarding.error.message.webhooks')));
                }
            },
            /**
            ...
            T::TO_WELCOME => static function (PostTransition $event) use ($container) {
                ...
                try {
                    $job->execute(Context::fromArray([]), new EphemeralJobRepository(), $container->get('paypal-pos.logger.woocommerce'));
