...
class SyncProgressView implements OnboardingView
{
     ...
     */
    public function __construct(callable $supportedWcProductsCountQuery, callable $totalWcProductsCountQuery, string $pluginTitle)
    {
    ...
    public function renderHeader(): string
    {
        ...
        <h2>
            <?php 
        esc_html_e('Initial sync in progress', 'paypal-point-of-sale');
        ?>
        </h2>
    ...
    public function renderContent(): string
    {
        ...
        <p>
            <?php 
        echo esc_html($this->stepDescriptionText());
        ?>
        </p>
        <div class="sync-navigation-note">
            <strong>
                <?php 
        esc_html_e('Please leave this window open until synchronization has finished.', 'paypal-point-of-sale');
        ...
        ?>
            ...
            <div class="sync-progress-text">
                <span class="sync-progress-message">
                    <?php 
        esc_html_e('Preparing to sync products', 'paypal-point-of-sale');
        ?>
                </span>
            ...
            <div class="sync-progress-action">
                <button
                    ...
                    name="cancel"
                    value="<?php 
        esc_attr_e('Cancel export', 'paypal-point-of-sale');
        ?>"
                >
                    <?php 
        esc_html_e('Cancel export', 'paypal-point-of-sale');
        ?>
                </button>
            </div>
        </div>
    ...
    {
        return wc_help_tip(sprintf(
            /** translators: %1$s: Shortname of the Plugin */
            esc_html__('If you navigate away during sync, you can always come back and continue.
                        However, %1$s will only start working correctly once it is finished.', 'paypal-point-of-sale'),
            $this->pluginTitle
        ));
    }
    public function renderProceedButton(): string
    {
        return $this->renderActionButton(ButtonAction::PROCEED, null, ['disabled' => \true, 'hidden' => \true]);
    }
    public function renderBackButton(): string
    {
        return $this->renderActionButton(ButtonAction::BACK, null, ['disabled' => \true]);
    }
    /**
    ...
    private function stepDescriptionText(): string
    {
        if ($this->supportedWcProductsCount() !== $this->totalWcProductsCount()) {
            return sprintf(
                ...
                __('%1$d WooCommerce products are now being synced to your PayPal Point of Sale library.', 'paypal-point-of-sale'),
                $this->supportedWcProductsCount()
            );
        }
        return __('All WooCommerce products are now being synced to your PayPal Point of Sale library.', 'paypal-point-of-sale');
    }
