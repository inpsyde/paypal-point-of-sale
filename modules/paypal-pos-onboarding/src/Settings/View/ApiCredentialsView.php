...
class ApiCredentialsView extends SettingsFieldView implements OnboardingView
{
     ...
     */
    public function __construct(WC_Settings_API $settingsApi, array $apiKeyCreationLink, string $title, string $content, array $fields, string $notice = '', array $allowedFieldTags = [])
    {
        $this->apiKeyCreationLink = $apiKeyCreationLink;
        parent::__construct($settingsApi, $title, $content, $fields, $notice, $allowedFieldTags);
    }
    ...
    public function renderContent(): string
    {
        ...
        <p>
            <?php 
        esc_html_e('2. Paste the key in the field below.', 'paypal-point-of-sale');
        ?>
        </p>
        ...
        foreach ($this->fields as $fieldId) {
            if (isset($this->settingsApi->form_fields[$fieldId])) {
                ...
                $fieldConfig['custom_attributes'] = $this->filterCustomAttributes($fieldConfig['custom_attributes']);
    ...
    {
        return $this->renderActionButton(ButtonAction::PROCEED, __('Authenticate with PayPal Point of Sale', 'paypal-point-of-sale'));
    }
    /**
    ...
    {
        $allowedList = ['autocomplete', 'required'];
        foreach ($customAttributes as $customAttribute => $value) {
            if (!in_array($customAttribute, $allowedList, \true)) {
