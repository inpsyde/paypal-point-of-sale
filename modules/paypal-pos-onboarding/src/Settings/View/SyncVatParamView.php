...
class SyncVatParamView implements OnboardingView
{
     ...
     */
    public function __construct(StoreComparison $storeComparison, StoreDataProvider $remoteStoreData, StoreDataProvider $localStoreData, ?array $defaultTaxRates)
    {
    ...
    public function renderHeader(): string
    {
        ...
        <h2>
            <?php 
        esc_html_e('Product prices', 'paypal-point-of-sale');
        ?>
        </h2>
        ...
        if (!$this->storeComparison->priceSyncRequiresTaxSync()) {
            ...
            <p>
                <strong>
                    <?php 
            esc_html_e('Your taxes will not be imported, you may need to configure them manually in your PayPal Point of Sale account.', 'paypal-point-of-sale');
            ?>
                </strong>
            </p>
        <?php 
        ...
        <?php 
        if ($this->defaultTaxRates !== null && empty($this->defaultTaxRates)) {
            ?>
            <div class="alert alert-warning">
                <?php 
            esc_html_e('You do not have default tax rates in your PayPal Point of Sale account, you may want to add them before syncing products.', 'paypal-point-of-sale');
            ?>
            </div>
        <?php 
        }
        ...
        <?php 
        if (!$this->storeComparison->canSyncPrices()) {
            ?>
            <p>
                <strong>
                    <?php 
            esc_html_e('Your prices will not be imported because your settings in PayPal Point of Sale and WooCommerce do not match:', 'paypal-point-of-sale');
            ?>
                </strong>
                <?php 
            $messages = [];
            if (!$this->storeComparison->currency()) {
                $messages[] = esc_html(sprintf(
                    /* translators: %1$s, %2$s: Currency codes (EUR, GBP, ...) */
                    __('Currency: %1$s in PayPal Point of Sale, %2$s in WooCommerce.', 'paypal-point-of-sale'),
            ...
            if ($this->storeComparison->priceSyncRequiresTaxSync()) {
                if (!$this->storeComparison->country()) {
                    $messages[] = esc_html(sprintf(
                        ...
                        __('Country: %1$s in PayPal Point of Sale, %2$s in WooCommerce.', 'paypal-point-of-sale'),
                        $this->remoteStoreData->country(),
                        $this->localStoreData->country()
                    ));
                }
                if (!$this->storeComparison->taxesEnabled()) {
                    $messages[] = esc_html(sprintf(__('Taxes are disabled in WooCommerce.', 'paypal-point-of-sale')));
        ...
        <?php 
        if (!$this->storeComparison->includeTaxes() && $this->storeComparison->canSyncPrices()) {
            ?>
            <div class="alert alert-warning">
                <?php 
            echo esc_html(sprintf(__('Remember that your tax inclusion settings in WooCommerce and PayPal Point of Sale do not match.
                            If you sync prices, the prices will be automatically adjusted to include/exclude taxes and your margins will change.', 'paypal-point-of-sale')));
            ?>
            </div>
        <?php 
        }
    ...
    public function renderContent(): string
    {
        ...
        <p>
            <?php 
            echo esc_html(sprintf(__('If you sync prices, your tax settings in WooCommerce and PayPal Point of Sale need to match.
                        If the sync is disabled, you can edit taxes in your PayPal Point of Sale library.', 'paypal-point-of-sale')));
            ?>
        </p>

        <p>
            <?php 
            esc_html_e('Please choose an option below.', 'paypal-point-of-sale');
            ?>
        </p>
            <?php 
        }
    ...
    public function renderFormChoiceSelection(): string
    {
        ...
        <div class="form-choice-selection">
            <div class="form-choice-selector<?php 
        echo $syncByDefault ? ' active' : '';
        ?>
                <?php 
        ...
        ?>
                ...
                <div class="form-choice-selector-content">
                    <label for="zettle-include-tax-prices">
                        <?php 
        esc_html_e('Sync prices', 'paypal-point-of-sale');
        ?>
                    </label>

                    <p class="form-choice-selector-content-description">
                        <?php 
        esc_html_e('Edit prices at WooCommerce to keep them up-to-date in PayPal Point of Sale.', 'paypal-point-of-sale');
        ?>
                    </p>
                </div>
            </div>
        ...
        ?>>
                ...
                <div class="form-choice-selector-content">
                    <label for="zettle-zero-prices">
                        <?php 
        esc_html_e("Don't sync prices", 'paypal-point-of-sale');
        ?>
                    </label>

                    <p class="form-choice-selector-content-description">
                        <?php 
        esc_html_e('Synced products will have the price set to 0. Update prices in your PayPal Point of Sale product library.', 'paypal-point-of-sale');
        ?>
                    </p>
                </div>
            </div>
    ...
    {
        return $this->renderActionButton(ButtonAction::PROCEED, __('Start sync', 'paypal-point-of-sale'));
    }
    /**
     * @inheritDoc
     */
