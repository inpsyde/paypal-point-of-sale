...
class OnboardingStepper
{
     ...
     */
    public function __construct(array $states, string $currentStep, array $exclude, string $stepLabel, string $stepsTemplate = '%1$d/%2$d')
    {
        $this->states = $states;
    ...
    {
        return in_array($this->currentStep, $this->steppableMap, \true);
    }
    /**
     * @return string
     */
    public function render(): string
    {
        ...
        ?>
            <div class="zettle-settings-onboarding-stepper">
                <?php 
        echo esc_html($this->generateStepper());
        ?>
            </div>
        <?php 
    ...
    protected function generateSteppableMap(array $states, array $exclude = []): array
    {
        ...
        foreach ($states as $state) {
            ...
            if (in_array($state->name(), $exclude, \true)) {
                continue;
            }
            $map[] = $state->name();
        }
    ...
    {
        return sprintf('%s %s', $this->stepLabel, sprintf($this->stepsTemplate, $this->currentPosition(), count($this->steppableMap) + 1));
    }
}
