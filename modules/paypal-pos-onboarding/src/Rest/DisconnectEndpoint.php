...
namespace Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\Onboarding\Rest;
...
class DisconnectEndpoint implements EndpointInterface
{
    ...
    {
        try {
            $this->onboardingResetJob->execute(Context::fromArray([]), new EphemeralJobRepository(), $this->logger);
        } catch (Throwable $exception) {
            $this->logger->error(sprintf('Error during reset job. %1$s', $exception->getMessage()));
            return new WP_REST_Response(['result' => ['success' => \false, 'error' => $exception->getMessage()]], 500);
        }
        return new WP_REST_Response(['result' => ['success' => \true]], 200);
    }
}
