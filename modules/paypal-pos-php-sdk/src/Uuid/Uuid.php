...
namespace Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\PhpSdk\Uuid;
...
class Uuid
{
    ...
    public static function fromWcProduct(WC_Product $product): self
    {
        ...
        $identifier = (int) ($dateCreated !== null ? $dateCreated->format('U') : 0) + (int) $product->get_id();
        //Salt simple products so that variants and products use different UUIDs even if they're the same in WC
        if ($product instanceof WC_Product_Simple) {
            $identifier += 1;
        }
        return new self(self::generate($identifier));
    }
    ...
    {
        if (PHP_INT_SIZE >= 8) {
            return str_pad(dechex($timestamp), 16, '0', \STR_PAD_LEFT);
        }
        return bin2hex(str_pad(self::toBinary((string) $timestamp), 8, "\x00", \STR_PAD_LEFT));
    }
    /**
     * @param string $digits
     *
