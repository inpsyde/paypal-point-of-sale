...
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\PhpSdk\DAL\Entity\Payment\MobilePayment;
...
class MobilePaymentBuilder implements MobilePaymentBuilderInterface
{
    ...
    {
        return ['uuid' => (string) $mobilePayment->uuid(), 'amount' => $mobilePayment->amount(), 'type' => $mobilePayment->type()->getValue()];
    }
    /**
     * @inheritDoc
     */
    ...
    {
        return $this->paymentFactory->createMobilePayment($data['uuid'], $data['type'], $data['amount']);
    }
}
