...
class CardPaymentBuilder extends AbstractBuilder implements CardPaymentBuilderInterface
{
    ...
    {
        $data = ['cardPaymentUUID' => (string) $cardPayment->uuid(), 'amount' => $cardPayment->amount(), 'type' => $cardPayment->type()->getValue(), 'attributes' => ['transactionStatusInformation' => '', 'cardPaymentEntryMode' => $cardPayment->cardPaymentEntryMode(), 'maskedPan' => $cardPayment->maskedPan(), 'referenceNumber' => $cardPayment->referenceNumber(), 'cardType' => $cardPayment->cardType()]];
    ...
    {
        return $this->paymentFactory->createCardPayment($data['uuid'], (float) $data['amount'], $data['attributes']['referenceNumber'], $data['attributes']['maskedPan'], $data['attributes']['cardType'], $data['attributes']['cardPaymentEntryMode'], $this->getDataFromKey('applicationName', $data['attributes']), $this->getDataFromKey('applicationIdentifier', $data['attributes']), $this->getDataFromKey('terminalVerificationResults', $data['attributes']), (int) $this->getDataFromKey('nrOfInstallments', $data['attributes']));
    }
}
