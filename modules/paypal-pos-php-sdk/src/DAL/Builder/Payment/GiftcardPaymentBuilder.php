...
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\PhpSdk\DAL\Entity\Payment\GiftCardPayment;
...
class GiftcardPaymentBuilder implements GiftcardPaymentBuilderInterface
{
    ...
    {
        return ['uuid' => (string) $giftCardPayment->uuid(), 'type' => $giftCardPayment->type()->getValue(), 'amount' => $giftCardPayment->amount()];
    }
    /**
     * @inheritDoc
     */
    ...
    {
        return $this->paymentFactory->createGiftcardPayment($data['uuid'], $data['type'], $data['amount']);
    }
}
