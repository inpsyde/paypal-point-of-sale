...
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\PhpSdk\DAL\Entity\Payment\VippsPayment;
...
class VippsPaymentBuilder implements VippsPaymentBuilderInterface
{
    ...
    {
        return ['uuid' => (string) $vippsPayment->uuid(), 'amount' => $vippsPayment->amount(), 'type' => $vippsPayment->type()->getValue()];
    }
    /**
     * @inheritDoc
     */
    ...
    {
        return $this->paymentFactory->createVippsPayment($data['uuid'], $data['type'], $data['amount']);
    }
}
