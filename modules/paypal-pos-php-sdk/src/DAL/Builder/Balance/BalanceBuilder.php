...
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\PhpSdk\DAL\Entity\Balance\Balance;
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\PhpSdk\DAL\Entity\Balance\BalanceFactory;
class BalanceBuilder implements BalanceBuilderInterface
{
    ...
    {
        return ['productUuid' => (string) $balance->productUuid(), 'variantUuid' => (string) $balance->variantUuid(), 'balance' => $balance->balance()];
    }
    /**
     * @param array $data
     *
    ...
    {
        return $this->balanceFactory->create($data['productUuid'], $data['variantUuid'], $data['balance']);
    }
}
