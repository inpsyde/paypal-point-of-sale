...
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\PhpSdk\DAL\Entity\Finance\PayoutInfo;
...
class PayoutInfoBuilder implements PayoutInfoBuilderInterface
{
    ...
    {
        return ['totalBalance' => $payoutInfo->totalBalance(), 'currencyId' => $payoutInfo->currencyId(), 'nextPayoutAmount' => $payoutInfo->nextPayoutAmount(), 'discountRemaining' => $payoutInfo->discountRemaining(), 'period' => $payoutInfo->period()->getValue()];
    }
    /**
     * @param array $data
     *
    ...
    {
        return $this->payoutInfoFactory->create($data['totalBalance'], $data['currencyId'], $data['nextPayoutAmount'], $data['discountRemaining'], $data['period']);
    }
}
