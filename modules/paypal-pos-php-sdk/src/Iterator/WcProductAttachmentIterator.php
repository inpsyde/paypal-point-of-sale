...
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\PhpSdk\Iterator\Attachment\ChildrenImageIterator;
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\PhpSdk\Iterator\Attachment\FeaturedImageIterator;
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\PhpSdk\Iterator\Attachment\GalleryImageIterator;
use WC_Product;
/**
...
class WcProductAttachmentIterator extends WcProductIteratorAggregate
{
    ...
    public function __construct(WC_Product $product, int $limit = 0, WcProductIterator ...$iterators)
    {
         ...
         */
        if (empty($iterators)) {
            $iterators = [new FeaturedImageIterator($product), new ChildrenImageIterator($product), new GalleryImageIterator($product)];
        }
