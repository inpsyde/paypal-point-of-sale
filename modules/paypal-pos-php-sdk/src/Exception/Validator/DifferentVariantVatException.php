...
{
    public function __construct(string $productName, array $vatValues, int $code = 0, ?Throwable $previous = null)
    {
        $vatStr = implode(', ', $vatValues);
        parent::__construct("The Product {$productName} has variant with different VAT values: [{$vatStr}].", $code, $previous);
    }
