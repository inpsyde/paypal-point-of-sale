...
namespace Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\PhpSdk\API\Products;
 ...
 *
 * @package Syde\PayPal\PointOfSale\PhpSdk\API\Products
 */
class Products
{
    /**
     ...
     */
    public function __construct(UriInterface $uri, RestClientInterface $restClient, BuilderInterface $builder, SerializerInterface $serializer, callable ...$listeners)
    {
        $this->uri = $uri;
     ...
     * @throws ZettleRestException
     */
    public function create(ProductInterface $product, bool $withListeners = \true): ProductInterface
    {
        $url = (string) $this->uri->withPath('/organizations/self/products')->withQuery('returnEntity=true');
     ...
     * @throws ZettleRestException
     */
    public function read(string $uuid, bool $withListeners = \true): ProductInterface
    {
    ...
    public function update(ProductInterface $product, bool $withListeners = \true): ProductInterface
    {
        ...
        $payload = $this->serializer->serialize($product);
        try {
            $result = $this->restClient->put($url, $payload, static function (RequestInterface $request) use ($etag): RequestInterface {
     ...
     * @throws ZettleRestException
     */
    public function delete(string $uuid, bool $withListeners = \true): bool
    {
        $url = (string) $this->uri->withPath("/organizations/self/products/{$uuid}");
        $success = \true;
     ...
     * phpcs:disable Generic.Metrics.NestingLevel.TooHigh
     */
    public function deleteBulk(array $products, bool $withListeners = \true): bool
    {
        if (empty($products)) {
            return \true;
        }
        $uuids = array_map(static function (ProductInterface $product): string {
            return $product->uuid();
        }, $products);
        $queryString = implode('&uuid=', $uuids);
        $url = (string) $this->uri->withPath("/organizations/self/products")->withQuery("uuid={$queryString}");
        $success = \true;
        $this->restClient->delete($url, []);
        if ($withListeners) {
