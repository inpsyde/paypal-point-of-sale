...
class ZettleAuthPlugin implements Plugin
{
    ...
    public function handleRequest(RequestInterface $request, callable $next, callable $first): Promise
    {
        ...
        return $next($request)->then(function (ResponseInterface $response) use ($request, $next, $first, $chainIdentifier, $authGrantType) {
            ...
            if ($status === 400 || $status === 401 || $status === 500) {
                ...
                $this->chainStorage[$chainIdentifier] = \true;
                /**
    ...
    private function handleAuthRequest(callable $first, RequestInterface $request, GrantType $grant): Promise
    {
        try {
            $data = array_merge(['grant_type' => $grant->type()], $grant->payload());
        } catch (InvalidTokenException $exception) {
            throw new AuthenticationException('Could not retrieve payload data', 0, $exception);
        }
        $pairs = [];
