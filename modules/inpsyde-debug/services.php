...
return ['inpsyde.debug.proxy-factory' => static function (C $ctr): DebugProxyFactory {
    return new DebugProxyFactory(...$ctr->get('inpsyde.debug.exception-handlers'));
    ...
    return [$ctr->get('inpsyde.debug.exception-handler.logging')];
}, 'inpsyde.debug.exception-handler.logging' => static function (C $ctr): ExceptionHandler {
    return new LogExceptionHandler($ctr->get('inpsyde.debug.logger'), $ctr->get('inpsyde.debug.exception-formatter'), $ctr->get('inpsyde.debug.exception-log-levels'));
    ...
    return new TypeDelegatingFormatter($ctr->get('inpsyde.debug.exception-formatter.fallback'), $ctr->get('inpsyde.debug.exception-formatters'));
}, 'inpsyde.debug.exception-formatter.fallback' => static function (C $ctr): ExceptionFormatter {
    return new FallbackFormatter($ctr->get('inpsyde.debug.is-debug-mode'));
