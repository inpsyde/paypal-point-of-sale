...
return ['paypal-pos.product.debug.listing.custom-column' => static function (C $container): CustomColumn {
    ...
    return new ProductsCommand($container->get('paypal-pos.sync.allowed-product-types'), $container->get('paypal-pos.sdk.id-map.product'), $container->get('paypal-pos.sdk.builder'), $container->get('paypal-pos.sdk.api.products'), $container->get('paypal-pos.sync.validator.product'), $container->get('paypal-pos.sync.status.matcher'));
...
}, 'paypal-pos.product.debug.rest.v1.endpoint.validate' => static function (C $container): EndpointInterface {
    return new ProductValidationEndpoint($container->get('paypal-pos.sync.validator.product'));
}, 'paypal-pos.product.debug.rest.v1.endpoint.validate.url' => static function (C $container): string {
    $endpoint = $container->get('paypal-pos.product.debug.rest.v1.endpoint.validate');
    return rest_url($container->get('paypal-pos.product.debug.rest.namespace') . $endpoint->route());
}];
