...
class ProductDebugModule implements ServiceModule, ExtendingModule, ExecutableModule
{
    ...
    public function run(C $container): bool
    {
        add_action('rest_api_init', static function () use ($container) {
            ...
            register_rest_route($container->get('paypal-pos.product.debug.rest.namespace'), $validateEndpoint->route(), ['methods' => $validateEndpoint->methods(), 'callback' => [$validateEndpoint, 'handleRequest'], 'permission_callback' => [$validateEndpoint, 'permissionCallback'], 'args' => $validateEndpoint->args()]);
        });
