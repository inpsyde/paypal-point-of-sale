...
return ['inpsyde.wc-lifecycle-events.products.listener-provider' => static function (ContainerInterface $container, ProductEventListenerRegistry $provider): ProductEventListenerRegistry {
     ...
     */
    $provider->onChange(static function (WC_Product $new, WC_Product $old) {
    ...
    $provider->onPublish(static function (WC_Product $new, WC_Product $old) {
        $success = 'A product was published!';
    });
    $provider->onTrash(static function (WC_Product $new, WC_Product $old) {
        $success = 'A product was moved to trash';
    });
    $provider->onDraft(static function (WC_Product $new, WC_Product $old) {
        $success = 'A product was moved to draft';
    });
    $provider->onDelete(static function (WC_Product $new, WC_Product $old) {
        $success = 'A product was deleted';
    });
    $provider->onPropertyChange('description', static function (WC_Product $new, WC_Product $old) {
        $oldDesc = $old->get_description();
        $newDesc = $new->get_description();
        $success = 'Description changed!';
    });
    $provider->onPropertyChange('stock_quantity', static function (WC_Product $new, WC_Product $old) {
        $oldStock = $old->get_stock_quantity();
        $newStock = $new->get_stock_quantity();
        $success = 'Stock changed!';
    });
    $provider->onTypeChange(static function (WC_Product $new) {
