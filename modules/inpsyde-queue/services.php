...
use Syde\Vendor\Zettle\Inpsyde\Queue\Db\QueueTable;
use Syde\Vendor\Zettle\Inpsyde\Queue\Log\ArrayLogger;
use Syde\Vendor\Zettle\Inpsyde\Queue\Log\WpCliLogger;
...
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\FileBasedLocker;
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\ItemsCountStopper;
...
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\Job\JobIterator;
...
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\Job\NullJob;
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\Job\WpDbJobRepository;
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\Locker;
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\Runner\AggregateRunner;
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\Runner\AsyncRequestRunner;
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\Runner\Runner;
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\Runner\WpCronRunner;
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\Runner\WpHeartbeatRunner;
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\Runner\WpShutdownRunner;
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\StoppableQueueWalker;
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\Stopper;
...
return [
    ...
    'inpsyde.queue.stopper.cli' => static function (C $container): Stopper {
        ...
        return new ItemsCountStopper($cron ? 1000000 : -1);
    },
    ...
    'inpsyde.queue.logger.array' => $wire(ArrayLogger::class, 'inpsyde.queue.logger'),
    /**
    ...
    'inpsyde.queue.locker' => static function (C $container): Locker {
        return new FileBasedLocker($container->get('inpsyde.queue.locker.timeout'), $container->get('inpsyde.queue.locker.file.path'));
    },
    'inpsyde.queue.runner' => static function (C $container): Runner {
        return new AggregateRunner(...$container->get('inpsyde.queue.runners'));
    },
    'inpsyde.queue.runners' => static function (C $container): array {
        $runners = [$container->get('inpsyde.queue.runner.heartbeat'), $container->get('inpsyde.queue.runner.wp-cron')];
    ...
    'inpsyde.queue.runner.wp-cron' => static function (C $container): Runner {
        return new WpCronRunner($container->get('inpsyde.queue.namespace'));
    },
    ...
    'inpsyde.queue.runner.can-run-on-shutdown' => static function (C $container): bool {
        ...
        // do not slow down customers, ajax requests, etc.
    },
    ...
    'inpsyde.queue.walker' => $wire(StoppableQueueWalker::class, 'inpsyde.queue.iterator', 'inpsyde.queue.stopper'),
    ...
    'inpsyde.queue.processor' => static function (C $container): QueueProcessor {
        ...
        return $builder->withRepository($container->get('inpsyde.queue.repository'))->withLocker($container->get('inpsyde.queue.locker'))->withLogger($container->get('inpsyde.queue.logger'))->withNetworkSupport($container->get('inpsyde.is-multisite'))->withExceptionHandler($container->get('inpsyde.queue.exception-handler'))->withMaxRetriesCount($container->get('inpsyde.queue.failed.retry.count'))->build();
    },
    'inpsyde.queue.create-job-record' => static function (C $container): callable {
        return static function (string $type, array $args = [], ?int $siteId = null) use ($container): JobRecord {
            $factory = $container->get('inpsyde.queue.factory');
            assert($factory instanceof JobRecordFactoryInterface);
            return $factory->fromData($type, Context::fromArray($args, $siteId));
        };
    },
    ...
    'inpsyde.queue.enqueue-job' => static function (C $container): callable {
        return static function (string $type, array $args = [], ?int $siteId = null, ?JobRepository $repository = null) use ($container): void {
    ...
    'inpsyde.queue.rest.v1.endpoint.process' => static function (C $container): EndpointInterface {
        return new ProcessEndpoint(new ProcessorBuilder($container->get('inpsyde.queue.factory')), $container->get('inpsyde.queue.repository'), $container->get('inpsyde.queue.locker'), $container->get('inpsyde.queue.logger.array'), $container->get('inpsyde.queue.rest.v1.endpoint.meta-callback'), $container->get('inpsyde.is-multisite'), $container->get('inpsyde.queue.failed.retry.count'));
    },
    'inpsyde.queue.rest.v1.endpoint.process.url' => static function (C $container): string {
        ...
        return rest_url($container->get('inpsyde.queue.rest.namespace') . $processEndpoint->route());
    },
];
