...
use Syde\Vendor\Zettle\Inpsyde\Queue\NetworkState;
...
use Syde\Vendor\Zettle\Inpsyde\Queue\Queue\UnstoppableQueueWalker;
...
class ProcessorBuilder
{
     ...
     * @return $this
     */
    public function withNetworkSupport(bool $state = \true): self
    {
    ...
    public function build(): QueueProcessor
    {
        ...
        if ($this->isMultisite) {
            $queueProcessor = new NetworkQueueProcessor($queueProcessor, static function (): NetworkState {
    ...
    {
        if ($this->stopper) {
            return new StoppableQueueWalker($jobIterator, $this->stopper);
        }
