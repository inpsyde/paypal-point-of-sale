...
class ExportProductJob implements Job
{
     ...
     */
    public function __construct(ProductRepositoryInterface $repository, BuilderInterface $builder, Products $productsClient, RemoteIdProvider $remoteIdProvider, QueueProcessor $processor, callable $createJobRecord, callable $isSyncable, callable $productStatus)
    {
    ...
    public function execute(ContextInterface $context, JobRepository $repository, LoggerInterface $logger): bool
    {
        ...
        if ($wcProduct instanceof WC_Product_Variation) {
            ...
            if ($wcProduct === null) {
                $logger->warning(sprintf("Could not find parent from Variation %s", $productId));
                return \true;
            }
        }
        // in some cases we may have the cached product instances,
        // so requesting to clear these caches to make sure that we are not sending incorrect/outdated data,
        // e.g. when building a product too early during new product publishing, it may not have the price.
        do_action('paypal-pos.clear-product-cache');
        if (!($this->isSyncable)($productId)) {
            $productStatus = ($this->productStatus)($productId);
            $logger->warning(sprintf('Skipping %s %s, not syncable because: %s.', get_class($wcProduct), $productId, implode(', ', $productStatus)));
            return \true;
        }
        try {
            $product = $this->builder->build(ProductInterface::class, $wcProduct);
        } catch (BuilderException $exception) {
            $logger->error(sprintf("Error while building PayPal Point of Sale Product from WooCommerce product %d. Skipping", $productId));
     ...
     */
    private function syncProduct(ProductTransferInterface $product, int $localProductId, LoggerInterface $logger, ?callable $onCreate = null): void
    {
        /**
     ...
     */
    private function attemptUpdate(ProductTransferInterface $product, int $localProductId, LoggerInterface $logger): void
    {
        try {
            try {
                ...
                if ($exception->isType(ZettleRestException::TYPE_ENTITY_NOT_FOUND)) {
                    $msg = sprintf("Could not find remote product %s despite local record being present. " . "This product appears to have been removed manually. Attempting to re-create it...", $product->uuid());
                    $logger->info($msg);
                    $this->processor->repository()->add(($this->createJobRecord)(UnlinkProductJob::TYPE, ['localId' => $localProductId]));
                    $this->processor->process();
                    throw new QueueRuntimeException($msg);
                    // will cause this job to retry
                }
                throw $exception;
            }
             ...
             */
            $this->logException($exception, $logger);
            $logger->warning(sprintf("Error while updating Product %s. Skipping...", $product->uuid()));
        }
    }
    /**
     ...
     *
     */
    private function afterCreate(WC_Product $product): void
    {
        if ($product instanceof WC_Product_Simple && !(bool) $product->managing_stock()) {
            return;
        }
        if ($product instanceof WC_Product_Variable && (!(bool) $product->managing_stock() && !$this->hasStockManagingVariations($product))) {
            return;
        }
        /**
         ...
         */
        $this->processor->repository()->add(($this->createJobRecord)(SetInventoryTrackingJob::TYPE, ['productId' => $product->get_id(), 'state' => \true]));
        $this->processor->repository()->add(($this->createJobRecord)(SyncStockJob::TYPE, ['productId' => $product->get_id(), 'frontOffice' => \false]));
    }
    /**
     ...
     * @throws QueueRuntimeException
     */
    private function attemptCreate(LazyProduct $product, LoggerInterface $logger, ?callable $onSuccess = null): void
    {
        try {
            /**
     ...
     * @param LoggerInterface $logger
     */
    private function removeProductIfExists(int $productId, LoggerInterface $logger): void
    {
        try {
        ...
        $logger->info(sprintf('Dispatching cleanup job for remote product %s and its mapping to local product %d', $remoteId, $productId));
        $this->processor->repository()->add(($this->createJobRecord)(DeleteProductJob::TYPE, ['productId' => $productId]));
    }
}
