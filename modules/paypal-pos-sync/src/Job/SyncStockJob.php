...
class SyncStockJob implements Job
{
    ...
    public function execute(ContextInterface $context, JobRepository $repository, LoggerInterface $logger): bool
    {
        ...
        try {
            ...
            $logger->info(sprintf('Attempting to sync inventory of %s %s with PayPal Point of Sale UUID %s', $wcProductClass, $productId, $productUuid));
        ...
        // Clear cache, otherwise we can get outdated WC stock numbers (if the product was accessed earlier during this request and updated in another request).
        if (apply_filters('paypal-point-of-sale.sync.clear-cache-before-stock-sync', \true)) {
        ...
        foreach ($this->fetchVariantUuids($wcProduct, $logger) as $variantUuid => $localId) {
            ...
            if ($stockChange === 0) {
                $logger->debug("Skipping product {$localId}: {$newStock} - {$remoteStock}.");
                continue;
                // already synced. Can happen if e.g. merging products in onboarding
            }
             ...
             */
            $transaction = $isFrontOffice ? $this->frontendTransaction($cappedStockChange) : $this->backendTransaction($cappedStockChange);
            $result = $this->syncVariant($productUuid, $variantUuid, $transaction, $logger);
            if (!$result) {
                return \false;
            }
            /**
            ...
            if ($cappedStockChange !== $stockChange) {
                $logger->info("This stock change {$stockChange} is larger than allowed by PayPal Point of Sale." . ' We will sync the rest in a new request');
                $this->execute(Context::fromArray(['productId' => $localId]), $repository, $logger);
            }
        }
        return \true;
    }
     ...
     */
    private function dispatchSetInventoryTrackingJob(WC_Product $wcProduct, LoggerInterface $logger, ContextInterface $context): void
    {
        ...
        $this->setInventoryTrackingJob->execute(Context::fromArray(['productId' => $productId, 'state' => \true]), new EphemeralJobRepository(), $logger);
        $logger->notice(sprintf('%s has finished self-healing', $this->type()));
    }
    /**
     ...
     * @return int
     */
    private function fetchRemoteStock(InventoryEntity $inventory, string $variantUuid): int
    {
        try {
    ...
    private function syncVariant(string $productUuid, string $variantUuid, array $transaction, LoggerInterface $logger): bool
    {
        try {
            $this->inventoryClient->moveStock($productUuid, $variantUuid, $transaction['from'], $transaction['to'], $transaction['change']);
            $logger->info(sprintf("Moved %s items from %s to %s", $transaction['change'], $transaction['from'], $transaction['to']), ['productUuid' => $productUuid, 'variantUuid' => $variantUuid]);
    ...
    {
        if ($change <= 0) {
            return ['from' => 'STORE', 'to' => 'SOLD', 'change' => abs($change)];
        }
        // This could be a refund
        return ['from' => 'SOLD', 'to' => 'STORE', 'change' => abs($change)];
    }
    private function backendTransaction(int $change): array
    {
        if ($change <= 0) {
            return ['from' => 'STORE', 'to' => 'BIN', 'change' => abs($change)];
        }
        return ['from' => 'SUPPLIER', 'to' => 'STORE', 'change' => abs($change)];
    }
    /**
    ...
    private function fetchVariantUuids(WC_Product $product, LoggerInterface $logger): array
    {
        ...
        if ($product->is_type(ProductType::VARIATION)) {
            ...
            if (!$parentProduct) {
                $logger->warning(sprintf('Could not find parent product %s for %s', $product->get_parent_id(), $product->get_id()));
                return [];
            }
        }
        try {
        ...
        if ($product->is_type(ProductType::VARIATION)) {
            ...
            foreach ($collection->all() as $variant) {
                if (in_array((string) $variant->uuid(), $variantUuids, \true)) {
