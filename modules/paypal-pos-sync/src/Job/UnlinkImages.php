...
class UnlinkImages implements Job
{
    ...
    public function execute(ContextInterface $context, JobRepository $repository, LoggerInterface $logger): bool
    {
        ...
        if ($attachmentId !== 0 && !empty($type) && !$this->fromAttachmentId($attachmentId, $type, $logger)) {
     ...
     * @param LoggerInterface $logger
     */
    private function removeFromVariation(WC_Product_Variation $variation, LoggerInterface $logger): void
    {
    ...
    private function cleanupVariants(WC_Product $product, LoggerInterface $logger): void
    {
        ...
        if (empty($localVariantIds)) {
            $logger->error("Cannot Unlink Variation Images, no Variations are found for product: {$product->get_id()}");
            return;
        }
     ...
     * @param LoggerInterface $logger
     */
    private function cleanupImage(string $type, int $attachmentId, string $remoteId, LoggerInterface $logger): void
    {
        try {
