...
class UnlinkVariantJob implements Job
{
    ...
    public function __construct(OneToOneMapInterface $variantMap)
    {
        if (!$variantMap instanceof MapRecordCreator) {
            throw new InvalidArgumentException(sprintf('Expected ID-Map of type %s to implement %s.', esc_html((string) get_class($variantMap)), MapRecordCreator::class));
        }
    ...
    private function removeWithVariationId(int $variationId, LoggerInterface $logger): bool
    {
        try {
            ...
            return \true;
            // No reason to retry this job even in case of failure
        }
    }
    /**
    ...
    private function removeWithVariantUuid(string $variantUuid, LoggerInterface $logger): bool
    {
        try {
            ...
            return \true;
            // No reason to retry this job even in case of a failure above
        }
    }
    /**
     * @param int $variantId
