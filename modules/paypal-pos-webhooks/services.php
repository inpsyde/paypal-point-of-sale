...
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\Webhooks\Handler\InventoryBalanceChangedHandler;
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\Webhooks\Handler\InventoryTrackingStartedHandler;
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\Webhooks\Handler\InventoryTrackingStoppedHandler;
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\Webhooks\Handler\LogHandler;
use Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\Webhooks\Handler\ProductDeletedHandler;
...
return [$job(InventoryBalanceChangedJob::TYPE) => static function (C $container): Job {
    ...
    return new WebhookRegistrationJob($container->get('paypal-pos.webhook.register'), $container->get('paypal-pos.webhook.can-register'));
}, 'paypal-pos.webhook.config' => static function (C $container): array {
    return ['uuid' => Uuid::v1(), 'eventNames' => $container->get('paypal-pos.webhook.config.events'), 'destination' => $container->get('paypal-pos.webhook.listener.url'), 'contactEmail' => $container->get('paypal-pos.webhook.config.email')];
    ...
    return ['ProductDeleted', 'InventoryBalanceChanged', 'InventoryTrackingStarted', 'InventoryTrackingStopped'];
    ...
    $url = str_replace('http://', 'https://', rest_url("{$namespace}{$route}"));
    ...
    return new WebhookListenerEndpoint($container->get('paypal-pos.webhook.logger'), $container->get('paypal-pos.sdk.api.webhooks.payload.factory'), ...$container->get('paypal-pos.webhook.handlers'));
}, 'paypal-pos.webhook.verifier' => static function (C $container): SignatureVerifier {
    return new SignatureVerifier($container->get('paypal-pos.webhook.signing-key'));
    ...
    return new WebhookRegistration($storage->fetch(), $container->get('paypal-pos.sdk.api.webhooks'), $container->get('paypal-pos.webhook.deletion'), $container->get('paypal-pos.webhook.logger'));
}, 'paypal-pos.webhook.deletion' => static function (C $container): WebhookDeletion {
    $storage = $container->get('paypal-pos.webhook.storage');
    assert($storage instanceof WebhookStorageInterface);
    return new WebhookDeletion($storage->fetch(), $container->get('paypal-pos.sdk.api.webhooks'), $container->get('paypal-pos.webhook.logger'), $container->get('paypal-pos.webhook.can-register'));
    ...
    return new WebhookStorage($container->get('paypal-pos.sdk.api.webhooks.factory'), $container->get('paypal-pos.webhook.storage.container'), $container->get('paypal-pos.webhook.storage.option'), $container->get('paypal-pos.webhook.config'));
...
}, 'paypal-pos.webhook.handlers.product-deleted' => static function (C $container) use ($job): WebhookHandler {
    return new ProductDeletedHandler($container->get($job(UnlinkProductJob::TYPE)), $container->get('paypal-pos.webhook.logger'));
}, 'paypal-pos.webhook.handlers.inventory-balance-changed' => static function (C $container) use ($job): WebhookHandler {
    return new InventoryBalanceChangedHandler($container->get($job(InventoryBalanceChangedJob::TYPE)), $container->get('paypal-pos.webhook.logger'), $container->get('paypal-pos.sdk.id-map.variant'), $container->get('paypal-pos.sdk.integration-id'));
}, 'paypal-pos.webhook.handlers' => static function (C $container): array {
    return [new LogHandler($container->get('paypal-pos.webhook.logger')), new InventoryTrackingStartedHandler(), new InventoryTrackingStoppedHandler(), $container->get('paypal-pos.webhook.handlers.inventory-balance-changed')];
    ...
    return static function () use ($container): bool {
        ...
        return !in_array($stateMachine->currentState()->name(), $noAuthStates, \true);
    };
}, 'paypal-pos.webhook.cli' => static function (C $container): WebhookCommand {
    return new WebhookCommand($container->get('paypal-pos.sdk.api.webhooks'), $container->get('paypal-pos.webhook.storage'), $container->get('paypal-pos.webhook.registration'));
}, 'paypal-pos.webhook.bootstrap' => static function (C $container): Bootstrap {
    return new Bootstrap($container->get('inpsyde.queue.enqueue-job'), $container->get('paypal-pos.webhook.delete'));
}];
