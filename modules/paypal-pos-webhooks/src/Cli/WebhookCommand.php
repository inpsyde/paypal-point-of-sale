...
namespace Syde\Vendor\Zettle\Syde\PayPal\PointOfSale\Webhooks\Cli;
...
class WebhookCommand
{
    ...
    public function __construct(Subscriptions $subscriptions, WebhookStorageInterface $webhookStorage, WebhookRegistration $webhookRegistration)
    {
    ...
    public function list()
    {
        try {
            ...
            $table = array_map(static function (ZettleWebhook $webhook): array {
                $status = 'UNREGISTERED';
                if ($webhook instanceof RegisteredWebhook) {
                    $status = $webhook->status();
                }
                return ['uuid' => $webhook->uuid(), 'eventNames' => json_encode($webhook->eventNames()), 'status' => $status, 'destination' => (string) $webhook->destination()];
            }, $result);
