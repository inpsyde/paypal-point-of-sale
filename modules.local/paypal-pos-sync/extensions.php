    ...
    'paypal-pos.settings.fields.registry' =>
        static function (array $previous, ContainerInterface $container): array {
            return array_merge(
                ...
                [
                    'sync_params' => [
                        'title' => __('Sync Parameters', 'paypal-point-of-sale'),
                        'type' => 'title',
                        'description' => __(
                            'Sets up how and what to synchronize to your PayPal Point of Sale store',
                            'paypal-point-of-sale'
                        ),
                    ],
                    'sync_price_strategy' => [
                        'title' => __('Price synchronization', 'paypal-point-of-sale'),
                        'type' => 'select',
                        'default' => PriceSyncMode::ENABLED,
                        'description' => __(
                            'Whether or not to sync prices to PayPal Point of Sale',
                            'paypal-point-of-sale'
                        ),
                        'options' => [
                            PriceSyncMode::ENABLED => __('Sync prices', 'paypal-point-of-sale'),
                            PriceSyncMode::DISABLED => __('Don\'t sync prices', 'paypal-point-of-sale'),
                        ],
                    ],
                    'sync_collision_strategy' => [
                        ...
                        'options' => [
                            ...
                            SyncCollisionStrategy::WIPE => __(
                                'Replace PayPal Point of Sale library',
                                'zettle-woocommerce'
                            ),
                        ],
                    ],
    ...
    'inpsyde.wc-lifecycle-events.products.listener-provider' =>
        static function (
            ...
            $registry->onPropertyChange(
                'stock_quantity',
                $container->get('paypal-pos.sync.listener.stock-quantity')
            );

            $registry->onPropertyChange(
                'manage_stock',
                $container->get('paypal-pos.sync.listener.manage-stock.simple'),
                $container->get('paypal-pos.sync.listener.manage-stock.variable'),
                $container->get('paypal-pos.sync.listener.manage-stock.variation')
            );

            $registry->onChange(
                $container->get('paypal-pos.sync.listener.all-props'),
                $container->get('paypal-pos.sync.listener.not-syncable'),
                $container->get('paypal-pos.sync.listener.variation.parent-stock'),
                $container->get('paypal-pos.sync.listener.delete-variable-without-variation')
            );

            $registry->onTypeChange(
                $container->get('paypal-pos.sync.listener.type-change.simple-to-variable'),
                $container->get('paypal-pos.sync.listener.type-change.variable-to-simple')
            );

            $registry->onDelete(
                $container->get('paypal-pos.sync.listener.delete.variation'),
            ...
            $registry->onPublish(
                $container->get('paypal-pos.sync.listener.publish.variation')
            );
