...
use Syde\PayPal\PointOfSale\ProductSettings\Metabox\ReadonlyMetaboxAction;
use Syde\PayPal\PointOfSale\ProductSettings\Metabox\ZettleProductLibraryLink;
use Syde\PayPal\PointOfSale\ProductSettings\Metabox\ZettleProductLibraryLinkView;
use Syde\PayPal\PointOfSale\ProductSettings\Provider\CustomProductTabProvider;
use Syde\PayPal\PointOfSale\ProductSettings\Provider\ProductExcludeProvider;
...
return [
    ...
    'paypal-pos.product-settings.taxonomy.sync-visibility.post-type' =>
        static function (C $container): string {
            return 'product';
        },
    'paypal-pos.product-settings.taxonomy.sync-visibility' =>
        static function (C $container): ZettleSyncVisibilityTaxonomy {
            return new ZettleSyncVisibilityTaxonomy(
                $container->get('paypal-pos.product-settings.taxonomy.sync-visibility.key'),
                $container->get('paypal-pos.product-settings.taxonomy.sync-visibility.post-type')
            );
        },
    ...
    'paypal-pos.product-settings.term.excluded' => static function (C $container): TermManager {
        ...
        return new TermManager(
            $container->get('paypal-pos.product-settings.term.excluded.name'),
            $container->get('paypal-pos.product-settings.term.excluded.slug'),
            $syncVisibilityTaxonomy->key()
        );
    },
    'paypal-pos.product-settings.product-settings-tab' =>
        static function (C $container): ProductSettingsTab {
            return new ProductSettingsTab(
                ...
                $container->get('paypal-pos.product-settings.barcode.input-field.simple'),
                $container->get('paypal-pos.product-settings.barcode.repository')
            );
        },
    'paypal-pos.product-settings.handler.exclude' =>
    ...
    'paypal-pos.product-settings.metabox.product.library.link.view' =>
        static function (C $container): BoxView {
            return new ZettleProductLibraryLinkView(
                $container->get('paypal-pos.product-settings.zettle.product.base-link')
            );
        },
    'paypal-pos.product-settings.metabox.product.library.link.action' =>
    ...
    'paypal-pos.product-settings.metabox.product.library.link' =>
        static function (C $container): ZettleProductLibraryLink {
            return new ZettleProductLibraryLink(
                ...
                $container->get('paypal-pos.product-settings.metabox.product.library.link.view'),
                $container->get('paypal-pos.product-settings.metabox.product.library.link.action'),
                $container->get('paypal-pos.product-settings.zettle.product.title')
            );
        },
    'paypal-pos.product-settings.provider.sync-visibility' =>
        static function (C $container): Provider {
            return new SyncVisibilityTaxonomyProvider(
                $container->get('paypal-pos.product-settings.taxonomy.sync-visibility')
            );
        },
    'paypal-pos.product-settings.provider.product-settings-tab' =>
        static function (C $container): Provider {
            return new CustomProductTabProvider(
                $container->get('paypal-pos.product-settings.product-settings-tab')
            );
        },
    'paypal-pos.product-settings.provider.product-exclude-handler' =>
        static function (C $container): Provider {
            return new ProductExcludeProvider(
                $container->get('paypal-pos.product-settings.handler.exclude')
            );
        },
    'paypal-pos.product-settings.provider' => static function (C $container): array {
        return [
            $container->get('paypal-pos.product-settings.provider.sync-visibility'),
            $container->get('paypal-pos.product-settings.provider.product-settings-tab'),
            $container->get('paypal-pos.product-settings.provider.product-exclude-handler'),
        ];
    },
    'paypal-pos.product-settings.is-product-editor' => static function (C $container): callable {
        return static function () use ($container): bool {
            ...
            if ($currentView === 'post-new') {
                ...
                return $container->get('paypal-pos.product-settings.product.is-product')(
                    $container->get('paypal-pos.product-settings.product-editor.product-from-url')()
                );
            }
    ...
    'paypal-pos.product-settings.product-editor.product-from-url' =>
        static function (C $container): callable {
            return static function (int $method = INPUT_GET): int {
                if (!in_array($method, [INPUT_GET, INPUT_POST, INPUT_COOKIE, INPUT_SERVER, INPUT_ENV], true)) {
                    $method = INPUT_GET;
                }
    ...
    'paypal-pos.product-settings.barcode.input-field.simple' => static function (
        C $container
    ): BarcodeInputField {
        return new BarcodeInputField(
            $container->get('paypal-pos.product-settings.barcode.input-field.name'),
            $container->get('paypal-pos.product-settings.barcode.repository'),
            __('Product barcode', 'paypal-point-of-sale'),
            'zettle-simple-product-barcode'
        );
    },
    'paypal-pos.product-settings.barcode.input-field.variation' => static function (
        C $container
    ): BarcodeInputField {
        return new BarcodeInputField(
            $container->get('paypal-pos.product-settings.barcode.input-field.name'),
            $container->get('paypal-pos.product-settings.barcode.repository'),
            __('Barcode', 'paypal-point-of-sale'),
            'zettle-variation-product-barcode form-field form-row'
        );
    },
    'paypal-pos.product-settings.barcode.repository' => static function (
        C $container
    ): BarcodeRepository {
        return new BarcodeRepository(
            $container->get('paypal-pos.product-settings.barcode.meta-key'),
            'paypal-point-of-sale.barcode.value'
        );
    },
    'paypal-pos.product-settings.barcode.save-handler.variation' => static function (
        C $container
    ...
    'paypal-pos.product-settings.barcode.standard-ui-enabled' => static function (): bool {
        return apply_filters('paypal-point-of-sale.barcode.standard-input-ui-enabled', true);
    },
];
