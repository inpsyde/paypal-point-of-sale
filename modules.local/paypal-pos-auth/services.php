...
return [
    'paypal-pos.oauth.token-storage.key' => static function (C $container): string {
        return 'api_token';
    },
    ...
    'paypal-pos.oauth.authentication' => $wire(
    ...
    'paypal-pos.oauth.credentials' => static function (C $container): ContainerInterface {
        return new CredentialsContainer(
            $container->get('paypal-pos.oauth.jwt.parser'),
            [],
            $container->get('paypal-pos.oauth.credentials.parent')
        );
    },
    ...
    'paypal-pos.oauth.grant.api' => $wire(
        JwtGrant::class,
        'paypal-pos.oauth.credentials',
        'paypal-pos.oauth.jwt.parser',
        'paypal-pos.oauth.client-id'
    ),
    'paypal-pos.http-plug.plugin' => static function (C $container): Plugin {
        return new ZettleAuthPlugin(
            $container->get('paypal-pos.oauth.authentication'),
            ...
            $container->get('paypal-pos.oauth.auth-grant'),
            $container->get('paypal-pos.oauth.refresh-grant'),
    ...
    'paypal-pos.oauth.http-client-factory' =>
        static function (C $container): AuthenticatedClientFactory {
            return new AuthenticatedClientFactory(
                ...
                $container->get('inpsyde.http-client.stream-factory'),
                $container->get('paypal-pos.oauth.jwt.parser'),
                $container->get('paypal-pos.oauth.headers.partner-affiliation'),
                $container->get('paypal-pos.oauth.client-id')
            );
        },
    'paypal-pos.oauth.credential-validator' => static function (C $container): CredentialValidator {
        return new CredentialValidator(
            $container->get('paypal-pos.oauth.http-client-factory'),
    ...
    'paypal-pos.oauth.headers.partner-affiliation' => static function (C $container): array {
        return [
            'X-iZettle-Application-Id' => $container->get('paypal-pos.oauth.client-id'),
        ];
    },
    ...
    'paypal-pos.oauth.jwt.rest.v1.endpoint.validate' =>
        static function (C $container): EndpointInterface {
            return new ValidationEndpoint(
                $container->get('paypal-pos.oauth.jwt.validator'),
