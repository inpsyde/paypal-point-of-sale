        ...
        if ($container->get('paypal-pos.assets.should-enqueue.all')()) {
            ...
            $previous[] = (new Script(
                    ...
                    static function () use ($container): array {
                        return [
                            'woocommerce_zettle_api_key' => [
                                'required' => [
                                    'message' => __('Enter the API key.', 'paypal-point-of-sale'),
                                ],
                                'remote' => [
                                    'url' => $container->get('paypal-pos.oauth.jwt.rest.url'),
                                    'valueParamName',
                                    'requestMethod' => 'GET',
                ...
                ->withLocalize(
                    ...
                    static function () use ($container): array {
                        return [
                            'url' => $container->get('paypal-pos.onboarding.disconnect.endpoint.url'),
                            'dialogId' => $container->get('paypal-pos.settings.account.disconnect.id'),
                            'nonce' => wp_create_nonce('wp_rest'),
                        ];
                    }
                );
                        ...
                        return [
                            ...
                            'messages' => [
                                ...
                                'status' => [
                                    ...
                                    'cleanup' => __('Cleaning up', 'paypal-point-of-sale'),
                                ],
                            ],
                            'jobTypes' => $jobTypes,
                        ];
